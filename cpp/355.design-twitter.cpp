 ...
 */
class Twitter {
  std::unordered_map<int, set<int>> followees;
  std::unordered_map<int, std::vector<pair<int,int>>> tweets;
  int order = 0;
public:
    Twitter() {
    ...
    vector<int> getNewsFeed(int userId) {
      ...
      for (const auto& user : followees[userId]) {
        for (auto tweet : tweets[user]) {
          news.push_back(tweet);
        }
      }

      std::sort(news.begin(), news.end(), std::greater());
      vector<int> res;
      int i = 0;
      for (auto [order, id] : news) {
        if (i >= 10) {
          break;
        }
        res.push_back(id);
