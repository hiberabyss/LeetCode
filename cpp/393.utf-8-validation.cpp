...
class Solution {
public:
  int get_byte_cnt(uint8_t data) {
    if ((data & 0x80) == 0) {
      return 1;
    }

    const std::unordered_map<uint8_t, uint8_t> prefix_cnt = {
      {0x6, 2},
      {0xe, 3},
      {0x1e, 4},
    };

    for (auto [prefix, cnt] : prefix_cnt) {
      if ((data >> (7 - cnt)) == prefix) {
        return cnt;
      }
    }
  ...
  bool is_sub_byte(uint8_t byte) {
    ...
    bool validUtf8(vector<int>& data) {
      int N = data.size();
      for (int i = 0; i < N; ) {
        int cnt = get_byte_cnt(data[i]);
        if (cnt == 0 || i + cnt > N) {
          return false;
        }

        for (size_t j = 1; j < cnt; ++j) {
          if (!is_sub_byte(data[i+j])) {
            return false;
          }
        }
        i += cnt;
