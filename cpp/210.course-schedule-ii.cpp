...
public:
  bool dfs(int v) {
    ...
    vector<int> findOrderBfs(int numCourses, vector<vector<int>>& prerequisites) {
      std::unordered_map<int, int> vertexes_indegree;

      for (const auto& edge : prerequisites) {
        edges[edge[1]].push_back(edge[0]);
        vertexes_indegree[edge[0]]++;
        // vertexes_indegree[edge[1]];
      }

      vector<int> starters;
      for (int i = 0; i < numCourses; i++) {
        if (vertexes_indegree[i] == 0) {
      ...
      while (!starters.empty()) {
        std::vector<int> next_starters;
        for (auto starter : starters) {
          visited.push_back(starter);
          for (auto neigh : edges[starter]) {
            vertexes_indegree[neigh]--;
            if (vertexes_indegree[neigh] == 0) {
