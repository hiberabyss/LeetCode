...
class Solution {
public:
    int dayName2Num(const string& day) {
        int res = 0;
        for (auto c : day) {
            if (isdigit(c)) {
                res *= 10;
    ...
    string reformatDate(string date) {
        static const map<string, int> nameNumMap = {
            {"Jan", 1 },
            {"Feb", 2 },
            {"Mar", 3 },
            {"Apr", 4 },
        ...
        sscanf(date.c_str(), "%s %s %d", &day, &month, &year);

        // std::cout << day << " " << month << std::endl;

        char buffer[100];
        sprintf(buffer, "%04d-%02d-%02d", year,
                nameNumMap.at(string(month)),
                dayName2Num(string(day)));
